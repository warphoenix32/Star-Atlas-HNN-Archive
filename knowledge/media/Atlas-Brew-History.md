...
seo_title: "Atlas Brew History: Star Atlas Transcripts and Institutional Discussions"
seo_description: "A source-critical history of Atlas Brew, its official community role, 123 preserved transcripts, selective semantic evidence, chronology gaps, and timestamp-based research use."
...
The significance review retained all 4,937 segments while sharply narrowing the higher-trust research queues. Promotion candidates fell from 3,306 to **659**, timeline candidates from 1,423 to **384**, and quote candidates from 1,218 to **193**. The revised method also reduced broad lexical `ROADMAP` and `SPECULATION` labels and recorded an exclusion reason for every rejected candidate. [Quality report](../../archive/semantic/atlas-brew/quality-report.json)

Every retained candidate points back to exact supporting captions, timestamps, a segment ID, and a source recording. This makes the corpus useful even when the speaker remains `UNKNOWN`: a researcher can locate the discussion and cue the external recording once its original URL is recovered. Candidate confidence measures extraction quality, not factual truth.

Speaker identity is treated as a dependency rather than a universal quality penalty. Of the 659 promotion candidates, 602 do not require speaker identity to preserve their bounded informational value; 20 depend on it partially; and 37 require it for the intended authority claim. A discussion can therefore support product-history research without supporting a named quotation. Claims such as "Michael Wagner committed the company to X" still require speaker-level evidence.
...
The current review classifies 659 segments as institutional-claim candidates, 70 as dateable-event discussions, 2,220 as contextual discussion, and 1,988 as low-value or background material. These are research dispositions, not canonical knowledge states.
...
`QUALIFIED`, risk class `R3`. Corpus preservation and candidate traceability are extensive. Chronology, original recording identity, and speaker-specific authority remain incomplete.
