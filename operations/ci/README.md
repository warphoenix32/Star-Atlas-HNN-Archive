...
Campaigns that declare `pipeline_version: "1.0"` in `campaign-report.json` are also checked against the simplified promotion contract. CI enforces risk-to-review mapping, R1 automatic-eligibility checks, citations, current-state dates, human-decision records, R4 authorization, R5 archive-only treatment, and human-controlled merge authority.
...
python operations/ci/promotion_contract.py operations/schema/examples/promotion-campaign-v1.json --root .
```
